    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code === 200 && res.data.userId > 0) {
                ...
                userDiv.innerHTML = '<img id="preview_image" src=' + res.data.avatar + '>' + '</img>' + '<span>' + res.data.username + '</span>';
...
function uploadImg() {
    $('#file_input').on('change', function () {
        ...
        // 使用FileReader对象读取文件并将其作为DataURL加载到图像标签中
    ...
    $.ajax({
        ...
        success: function (body) {
            ...
            for (let friend of body.data) {
                ...
                namediv.innerHTML = '<h4>' + friend.friendName + '</h4>';
                li.innerHTML = '<img src=' + friend.friendPhoto + '>' + '</img>';
...
function clickFriend(friend) {
    ...
    if (sessionLi) {
        ...
        //创建会话  并在会话li中填充信息
        sessionLi = $("<li>");
        sessionLi.html('<img src=' + friend.friendPhoto + '>' + '</img>' +
            '<div>' +
...
function findSessionByName(username) {
    ...
    for (var i = 0; i < sessionLis.length; i++) {
        ...
        var h3 = sessionLi.find('div>h3');
        if (h3.text().trim() === username) {
            return sessionLi;
        }
    }
    return null;
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.code == 200 && res.data.length >= 0) {
                ...
                let liItems = sessionListUL.querySelectorAll('li');
                // 遍历所有 li 元素
                liItems.forEach(item => {
                    // 如果 li 元素不包含类名 'fri_req'，则移除它
                    if (!item.classList.contains('fri_req')) {
                        sessionListUL.removeChild(item);
                    }
                });
                // 2. 遍历响应的数组, 针对结果来构造页面
                for (let session of res.data) {
                    ...
                    infoDiv.innerHTML = '<h3>' + session.friends[0].friendName + '</h3>'
                        + '<p>' + session.lastMessage + '</p>';
                    li.innerHTML = '<img src=' + session.friends[0].friendPhoto + '>' + '</img>';
...
function getHistoryMessage(sessionId) {
    ...
    $.ajax({
        type: 'get',
        url: 'message?sessionId=' + sessionId + '&limit=' + limit,
        success: function (res) {
            if (res.code == 200 && res.data.length > 0) {
...
function addMessage(messageShowDiv, message) {
    ...
    var selfUsername = $('.left .user>span').html();
    ...
    jQuery.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 200 && res != null) {
                jQuery("#preview_image").attr('src', res.data.avatar);
            }
        }
    })
}
